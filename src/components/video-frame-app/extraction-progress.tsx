...
export function ExtractionProgress() {
  ...
  const elapsed =
    extractionStartTime && now ? Math.max(0, (now - extractionStartTime) / 1000) : 0;
    ...
    <div className="w-full" role="status" aria-live="polite">
      ...
      {extractionStatus === "cancelled" && (
        <div className="flex items-center gap-3 p-4 bg-muted/40 rounded-lg border border-border">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium">Extraction Cancelled</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {framesExtracted.toLocaleString()} frames were extracted before cancelling.
            </p>
          </div>
        </div>
      )}
